...
def main():
    ...
    class ImageTextDataset(torch.utils.data.Dataset):
        ...
        def __init__(self, image_folder, tokenizer, transforms, args):
    ...
    with accelerator.main_process_first():
        train_dataset = ImageTextDataset(args.image_folder, tokenizer, train_transforms, args)
    ...
    def collate_fn(examples):
        ...
        captions = torch.cat([example["captions"] for example in examples])
        attention_mask = torch.cat([example["attention_mask"] for example in examples])

        return {"pixel_values": image, "input_ids": captions, "prompt_attention_mask": attention_mask}
